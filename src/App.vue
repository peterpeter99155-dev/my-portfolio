...
<head>
    ...
    <title>資產戰略總部 v8.3 - 全球戰略總指揮 (終極圖鑑版)</title>
    <script src="https://unpkg.com/vue@3/dist/vue.global.js"></script>
    <script src="https://cdn.tailwindcss.com"></script>
    <script src="https://cdn.jsdelivr.net/npm/echarts@5.5.0/dist/echarts.min.js"></script>
    <style>
        input::-webkit-outer-spin-button, input::-webkit-inner-spin-button { -webkit-appearance: none; margin: 0; }
        .font-mono-data { font-family: 'ui-monospace', 'SFMono-Regular', 'Menlo', 'Monaco', 'Consolas', monospace; }
        .custom-scrollbar::-webkit-scrollbar { width: 8px; }
        .custom-scrollbar::-webkit-scrollbar-track { background: #0f172a; border-radius: 4px; }
        .custom-scrollbar::-webkit-scrollbar-thumb { background: #334155; border-radius: 4px; }
        .custom-scrollbar::-webkit-scrollbar-thumb:hover { background: #475569; }
        .input-pv { background-color: #1e293b; border: 1px solid #334155; transition: all 0.2s; }
        .input-pv:focus { border-color: #4f46e5; background-color: #0f172a; outline: none; box-shadow: 0 0 0 1px rgba(79, 70, 229, 0.5); }
        .currency-toggle { appearance: none; background-image: url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 20 20'%3e%3cpath stroke='%2394a3b8' stroke-linecap='round' stroke-linejoin='round' stroke-width='1.5' d='M6 8l4 4 4-4'/%3e%3c/svg%3e"); background-position: right 0.2rem center; background-repeat: no-repeat; background-size: 1.2em 1.2em; }
        [v-cloak] { display: none; }
    </style>
</head>
<body class="bg-slate-950 text-slate-100 p-4 md:p-8 font-sans min-h-screen">
    <div id="app" v-cloak class="max-w-[1600px] mx-auto space-y-6" @click="closeAllSearch">
        ...
        <div class="bg-slate-900 rounded-2xl border-2 border-slate-800 overflow-hidden shadow-xl">
            ...
            <div v-for="c in [usdCash, twdCash]" class="grid grid-cols-12 gap-4 p-4 bg-slate-900/30 border-b border-slate-800/50 items-center">
                    ...
                    <div class="col-span-5 relative flex gap-2">
                        ...
                        <div class="flex-1 relative">
                            ...
                            <ul v-if="item.showSearch && filteredDict(item.name).length > 0" class="absolute z-50 w-full bg-slate-800 border border-slate-600 rounded-md shadow-2xl top-[100%] mt-1 max-h-48 overflow-y-auto custom-scrollbar">
                                <li v-for="d in filteredDict(item.name)" @mousedown.prevent="selectStock(item, d)" class="p-2 hover:bg-indigo-600 cursor-pointer text-xs font-bold text-slate-200 border-b border-slate-700 last:border-0 truncate">{{ d }}</li>
                            </ul>
                        </div>
                    </div>
    ...
    <script>
        ...
        createApp({
            data() {
                return {
                    thresholdTWD: 10000, fx: { buyRate: 31.5, currentRate: 32.0 },
            ...
            computed: {
                getTWDValue() { return (item) => { const v = parseFloat(item.current) || 0; return item.currency === 'USD' ? v * this.fx.currentRate : v; }; },
                totalAssetTWD() { return [this.usdCash, this.twdCash, ...this.coreList, ...this.satelliteList].reduce((s, i) => s + this.getTWDValue(i), 0); },
                totalTargetPct() { return [this.usdCash, this.twdCash, ...this.coreList, ...this.satelliteList].reduce((s, i) => s + (parseFloat(item.target) || 0), 0); }, // 修正語法
                totalTargetPct() { const all = [this.usdCash, this.twdCash, ...this.coreList, ...this.satelliteList]; return all.reduce((s, i) => s + (parseFloat(i.target) || 0), 0); },
                isAllocationValid() { return Math.abs(this.totalTargetPct - 100) < 0.01; },
                fxProfit() { return this.totalAssetTWD - [this.usdCash, this.twdCash, ...this.coreList, ...this.satelliteList].reduce((s, i) => s + (i.currency === 'USD' ? (parseFloat(i.current)||0) * this.fx.buyRate : (parseFloat(i.current)||0)), 0); },
                rebalanceActions() {
            ...
            methods: {
                ...
                addRow(type) { const row = { name: '', currency: type === 'core' ? 'USD' : 'TWD', current: 0, target: 0, showSearch: false }; if(type === 'core') this.coreList.push(row); else this.satelliteList.push(row); },
                removeRow(type, idx) { if(confirm('確認移除？')) { if(type === 'core') this.coreList.splice(idx, 1); else this.satelliteList.splice(idx, 1); this.updateChart(); } },
                initChart() { this._chart = echarts.init(document.getElementById('echart-container')); window.addEventListener('resize', () => this._chart.resize()); },
                updateChart() {
                    if(!this._chart) return;
                    const data = [];
                    const colors = ['#3b82f6', '#60a5fa', '#818cf8', '#93c5fd', '#f59e0b', '#fbbf24', '#fcd34d', '#fde68a'];
                    this.coreList.concat(this.satelliteList).forEach((i, idx) => { const v = this.getTWDValue(i); if(v > 0) data.push({ value: v, name: i.name || '未命名', itemStyle: { color: colors[idx % 8] } }); });
                    ...
                    this._chart.setOption({ tooltip: { trigger: 'item', formatter: '{b}<br/>NT$ {c} ({d}%)' }, series: [{ type: 'pie', radius: ['35%', '60%'], label: { show: true, formatter: '{name|{b}}\n{perc|{d}%}', rich: { name: { color: '#f1f5f9', fontSize: 11, fontWeight: 'bold' }, perc: { color: '#94a3b8', fontSize: 10 } } }, data }] });
                    this.save();
                },
                save() { localStorage.setItem('portfolio_v8_public_template', JSON.stringify({ fx: this.fx, thresholdTWD: this.thresholdTWD, usdCash: this.usdCash, twdCash: this.twdCash, coreList: this.coreList.map(i=>({name:i.name, currency:i.currency, current:i.current, target:i.target})), satelliteList: this.satelliteList.map(i=>({name:i.name, currency:i.currency, current:i.current, target:i.target})) })); }
            },
            mounted() {
                const saved = localStorage.getItem('portfolio_v8_public_template');
                if(saved) { try { const p = JSON.parse(saved); Object.assign(this, p); this.coreList.forEach(i => i.showSearch = false); this.satelliteList.forEach(i => i.showSearch = false); } catch(e) {} }
                this.$nextTick(() => { setTimeout(() => { this.initChart(); this.updateChart(); }, 50); });
            }
        }).mount('#app')
    </script>
</body>
